...
public class EventoDAO implements IEventoDAO{
    ...
    private final String stmtListarEventosUsuario = "SELECT * FROM EVENTO WHERE " + colidusuario + " = ?";
    ...
    public void removerEvento(int idEvento) throws Exception {
        try {     
            ...
            //remove todos os comentarios do evento
            ComentarioDAO comentarioDAO = new ComentarioDAO();
            List<Comentario> comentarios = comentarioDAO.listarComentariosEvento(idEvento);
            for (int i = 0 ; i < comentarios.size() ; i++)
                comentarioDAO.removerComentario(comentarios.get(i).getIdComentario());
            
            //remove todos os itens do evento
            ItemDAO itemDAO = new ItemDAO();
            List<Item> itens = itemDAO.listarItensEvento(idEvento);
            for (int i = 0 ; i < itens.size() ; i++)
                itemDAO.removerItem(itens.get(i).getIdItem());
    ...
    List<Evento> listarEventosUsuario(int idUsuario) throws Exception {
        try {
            
            List<Evento> eventos = new ArrayList<>();            
            
            con = new ConnectionFactory().getConnection();
            stmt = con.prepareStatement(stmtListarEventosUsuario);
