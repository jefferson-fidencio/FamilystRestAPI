...
public class UsuarioDAO implements IUsuarioDAO{
    ...
    private final String stmtBuscarUsuarioPorEmail = "SELECT * FROM USUARIO WHERE " + colemail + " = ?";
    ...
    public void removerUsuario(int idUsuario) throws Exception {
        try {     
            ...
            //remove todos os comentarios do usuario
            ComentarioDAO comentarioDAO = new ComentarioDAO();
            List<Comentario> comentarios = comentarioDAO.listarComentariosUsuario(idUsuario);
            for (int i = 0 ; i < comentarios.size() ; i++)
                comentarioDAO.removerComentario(comentarios.get(i).getIdComentario());
            
            //remove todos as noticias do usuario
            NoticiaDAO noticiaDAO = new NoticiaDAO();
            List<Noticia> noticias = noticiaDAO.listarNoticiasUsuario(idUsuario);
            for (int i = 0 ; i < noticias.size() ; i++)
                noticiaDAO.removerNoticia(noticias.get(i).getIdNoticia());
            
            //remove o usuario das suas familias
            FamiliaDAO familiaDAO = new FamiliaDAO();
            List<Familia> familias = familiaDAO.listarFamiliasUsuario(idUsuario);
            for (int i = 0 ; i < familias.size() ; i++)
                familiaDAO.removerUsuarioFamilia(idUsuario, familias.get(i).getIdFamilia());
            
            //remove todos os eventos do usuario
            EventoDAO eventoDAO = new EventoDAO();
            List<Evento> eventos = eventoDAO.listarEventosUsuario(idUsuario);
    ...
    public Usuario buscarUsuarioPorEmail(String emailUsuario) throws Exception {
        try {         
            ...
            con = new ConnectionFactory().getConnection();
            stmt = con.prepareStatement(stmtBuscarUsuarioPorEmail);
            ...
            while (rs.next()) {
                Timestamp timestamp = rs.getTimestamp(coldataCriacao);
                java.util.Date dataCriacao = new java.util.Date(timestamp.getTime());   
                usuario = new Usuario(rs.getInt(colid), rs.getString(colnome), "", rs.getString(colidFacebook), rs.getString(collocal), rs.getString(colemail), dataCriacao);               
            }        
