...
public class ComentarioDAO implements IComentarioDAO{
    ...
    private final String stmtListarComentariosNoticia = "SELECT * FROM COMENTARIO WHERE " + colidNoticia + " = ?";
    private final String stmtListarComentariosUsuario = "SELECT * FROM COMENTARIO WHERE " + colidusuario + " = ?";
    private final String stmtInserirComentarioNoticia = "INSERT INTO COMENTARIO (" + coldados + "," + coldataCriacao + "," + colidNoticia+ "," + colidusuario + ") VALUES (?,NOW(),?,?)";
    private final String stmtInserirComentarioEvento = "INSERT INTO COMENTARIO (" + coldados + "," + coldataCriacao + "," + colidevento+ "," + colidusuario + ") VALUES (?,NOW(),?,?)";
    ...
    @Override
    public List<Comentario> listarComentariosNoticia(int idNoticia) throws Exception {
        try {
            
            List<Comentario> comentarios = new ArrayList<>();            
            
            con = new ConnectionFactory().getConnection();
            stmt = con.prepareStatement(stmtListarComentariosNoticia);
    ...
    public int inserirComentario(Comentario comentario) throws Exception {
        try {        
            ...
            //insere comentario
            con = new ConnectionFactory().getConnection();
            if (comentario.getIdEvento() != 0)
                stmt = con.prepareStatement(stmtInserirComentarioEvento, Statement.RETURN_GENERATED_KEYS);
            else if (comentario.getIdNoticia() !=  0) 
                stmt = con.prepareStatement(stmtInserirComentarioNoticia, Statement.RETURN_GENERATED_KEYS);
            stmt.setString(1, comentario.getDescricao());  
            if (comentario.getIdEvento() != 0)          
                stmt.setInt(2, comentario.getIdEvento());              
            else if (comentario.getIdNoticia() !=  0) 
                stmt.setInt(2, comentario.getIdNoticia()); 
    ...
    List<Comentario> listarComentariosUsuario(int idUsuario) throws Exception {
        try {
            
            List<Comentario> comentarios = new ArrayList<>();            
            
            con = new ConnectionFactory().getConnection();
            stmt = con.prepareStatement(stmtListarComentariosUsuario);
