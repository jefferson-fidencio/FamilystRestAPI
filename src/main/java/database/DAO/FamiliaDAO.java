...
public class FamiliaDAO implements IFamiliaDAO{
    ...
    private final String coladmin = "administrador";
    ...
    private final String stmtInserirUsuarioFamilia = "INSERT INTO USUARIO_HAS_FAMILIA (" + colidusuario + "," + colidfamilia + "," + coladmin + ") VALUES (?,?,1)";
    private final String stmtRemoverUsuarioFamilia = "DELETE FROM USUARIO_HAS_FAMILIA WHERE " + colidusuario + " = ? AND " + colidfamilia + " = ?";
    private final String stmtRemoverUsuariosFamilia = "DELETE FROM USUARIO_HAS_FAMILIA WHERE " + colidfamilia + " = ?";
    ...
    public void removerFamilia(int idFamilia) throws Exception {
        try {     
            ...
            if ( familiaExistente == null)
             ...
             //remove a familia de todos usuarios
            con = new ConnectionFactory().getConnection();
            stmt = con.prepareStatement(stmtRemoverUsuariosFamilia);
            stmt.setInt(1, idFamilia);
            stmt.executeUpdate();
                        
            //remove albuns da familia
            AlbumDAO albumDAO = new AlbumDAO();
            List<Album> albuns = albumDAO.listarAlbunsFamilia(idFamilia);
            for (int i = 0 ; i < albuns.size() ; i++)
                albumDAO.removerAlbum(albuns.get(i).getIdAlbum());
            
            //remove todos as noticias da familia
            NoticiaDAO noticiaDAO = new NoticiaDAO();
            List<Noticia> noticias = noticiaDAO.listarNoticiasFamilia(idFamilia);
            for (int i = 0 ; i < noticias.size() ; i++)
                noticiaDAO.removerNoticia(noticias.get(i).getIdNoticia());
            
            //remove todos os eventos do usuario
            EventoDAO eventoDAO = new EventoDAO();
            List<Evento> eventos = eventoDAO.listarEventosFamilia(idFamilia);
            for (int i = 0 ; i < eventos.size() ; i++)
                eventoDAO.removerEvento(eventos.get(i).getIdEvento());
    ...
    public void inserirUsuarioFamilia(Usuario usuario, int idFamilia) throws Exception{
        try {        
            ...
            con = new ConnectionFactory().getConnection();
            stmt = con.prepareStatement(stmtInserirUsuarioFamilia, Statement.RETURN_GENERATED_KEYS);
    ...
    public void removerUsuarioFamilia(int idUsuario, int idFamilia) throws Exception {
        try {     
            ...
            con = new ConnectionFactory().getConnection();
            stmt = con.prepareStatement(stmtRemoverUsuarioFamilia);
            stmt.setInt(1, idUsuario);
            stmt.setInt(2, idFamilia);
            stmt.executeUpdate();
